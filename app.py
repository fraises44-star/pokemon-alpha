...
# ---------------------------------------------------------
# API REQUEST FOR PRICE TIER
# ---------------------------------------------------------

@st.cache_data(ttl=3600)
def get_cards_for_tier(min_price, max_price):

    query = (
        f"cardmarket.prices.trendPrice:"
...
st.markdown(
    "## 🏆 Best Investment Candidate by Price Tier"
)
...
for min_price, max_price in price_tiers:
    ...
    for card in cards:
        ...
        low_price = prices.get(
            "lowPrice"
        )

        average_sell = prices.get(
            "averageSellPrice"
        )
        ...
        # Avoid overlap between tiers
        if min_price == 0:

            inside_range = (
                0 < trend_price < max_price
            )

        else:

            inside_range = (
                min_price <= trend_price < max_price
            )
...
if tier_results:
    ...
    for column, card in zip(
        ...
        with column:
            ...
            st.markdown(
                f"### €{int(card['Min Price'])}"
                f"–€{int(card['Max Price'])}"
            )
    ...
    st.warning(
        "Could not retrieve price tier recommendations."
    )
...
if search:
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            st.error(
                "The Pokémon API returned an error."
            )
        ...
        st.error(
            "Could not connect to the Pokémon card API."
        )
