...
def score_card(card):
    ...
    if low_price and trend_price:
        ...
        discount = ((trend_price - low_price) / trend_price) * 100
    ...
    if average_sell and trend_price:
        ...
        difference = abs(average_sell - trend_price) / trend_price
...
if tier_results:
    ...
    for column, card in zip(first_row, tier_results[:5]):
        ...
        with column:
            ...
            st.write(recommendation(card["PII"]))
    ...
    for column, card in zip(second_row, tier_results[5:]):
...
if search:
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            st.error(
                f"Search API error: {response.status_code} — {response.text[:500]}"
            )
